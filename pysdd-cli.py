...
import argparse
import logging
from pysdd.sdd import SddManager, Vtree, WmcManager, Fnf, CompilerOptions
...
def create_wmc(node, weights, args):
    ...
    for i in range(len(weights)):
        ...
        if (i % 2) == 1:
            l = -l
        w = weights[i]
        wmc.set_literal_weight(node.manager().literal(l), w)
    return wmc
...
def print_node(node, manager, wmc):
...
def getopt(argv=None):
    BytesPath = lambda p: bytes(Path(p))
    BytesStr = lambda s: s.encode()
    ...
    input_group.add_argument('-c', metavar='FILE', type=BytesPath,
                             dest='cnf_filename', help='set input CNF file')
    input_group.add_argument('-d', metavar='FILE', type=BytesPath,
                             dest='dnf_filename', help='set input DNF file')
    input_group.add_argument('-s', metavar='FILE', type=BytesPath,
                             dest='sdd_filename', help='set input SDD file')
    parser.add_argument('-v', metavar='FILE', type=BytesPath,
                        dest='vtree_filename', help='set input VTREE file')

    parser.add_argument('-W', metavar='FILE', type=BytesPath,
                        dest='output_vtree_filename', help='set output VTREE file')
    parser.add_argument('-V', metavar='FILE', type=BytesPath,
                        dest='output_vtree_dot_filename', help='set output VTREE (dot) file')
    parser.add_argument('-R', metavar='FILE', type=BytesPath,
                        dest='output_sdd_filename', help='set output SDD file')
    parser.add_argument('-S', metavar='FILE', type=BytesPath,
    ...
    parser.add_argument('-t', metavar='TYPE', type=BytesStr, default='balanced',
